...
A Wikipedia article about programming languages exists which contains a list of 664 different programming language. This tells that constructing a programming language is definitely not a new thing to do. Fortunate enough though, because doing so could at first seem like an overwhelming task, but luckily we can draw knowledge from experienced language designers who have come up with methods which can be used for creating a compiler or interpreter for a programming language. This section describes how it is commonly done and the phases involved, which we use ase a guideline for the construction of \productname{}.
...
\begin{tikzpicture}
  ...
  \draw[tuborg, decoration={brace}] let \p1=(leana.south), \p2=(syana.north) in
    ($(2, \y1)$) -- ($(2, \y2)$) node[tubnode] {Tokens};
  
  \draw[tuborg, decoration={brace}] let \p1=(syana.south), \p2=(seana.north) in
    ($(2, \y1)$) -- ($(2, \y2)$) node[tubnode] {Parse tree};
  
  \draw[tuborg, decoration={brace}] let \p1=(seana.south), \p2=(cgen.north) in
    ($(2, \y1)$) -- ($(2, \y2)$) node[tubnode] {Intermediate code};
  
  \draw[tuborg, decoration={brace}] let \p1=(cgen.south), \p2=(mach.north) in
...
\begin{table}
  %% TODO FIX THIS TABLE AND CREATE MACRO FOR CONSISTENCY
\center
...
\fig[scale=0.5]{compileroverviewinterpretation}{The different phases of an interpreter. Based on Sebesta \textit{et al.}\cite{sebesta2013} p. 48, Figure 1.4}
...
\fig[scale=0.5]{compileroverviewhybrid}{The different phases of a hybrid implementation systems. Based on Sebesta \textit{et al.}\cite{sebesta2013} p. 49, Figure 1.5}
