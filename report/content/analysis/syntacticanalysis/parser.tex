...
example of an LL($1$)-parser can be seen in \tableref{table:LL1}. In the
...
\tab[11cm]{LL1}{3}{An LL($1$) parser in action parsing the string ``int + int''.}
	      {The process                                          }
...
LR-parsers also have two actions: shift and reduce. The shift action
...
LR($2$)-parser in action is illustrated in \tableref{table:LR2}.

\tab[11cm]{LR2}{3}{An LR($2$) parser seen in action parsing the string ``int + int''.}
	  {The process	    					 }
{Step  	 }{Production & Input       & Action                     }{
...
how to construct a parser in \secref{subsec:constructingaparser}.
