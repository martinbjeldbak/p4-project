...
In \lstref{lst:scan} you can clearly see many functions with the name \methodref{is{Something}()} for instace the isDigit() function. These functions check the first character in an input and checks if the next symbol in the input stream is that ``{Something}''. \methodref{isWhitespace()} checks if the next input character (\varref{nextChar}) is a white space. While true \methodref{pop()} dequeues the next symbol in the input stream and updates \varref{nextChar}. Therefore, the while loop with \methodref{isWhitespace()} removes all initial white spaces before a next token is found. If, at any time, the scanner has reached the end of the input stream it returns a \tokenref{EOF} so succeeding parts of the parser knows where the program ends.

Beside the \methodref{isWhitespace()} function, all other functions of the form \methodref{is{Something}()} are reponsible for returning a token based on their evaluation of the input stream.
...
\tabrow{\$    			}{scanVar()				& VAR									}
\tabrow{Operator (e.g. ``+'', ``-'' and ``*'')   	}{scanOperator()		& LBRACKET, RBRACKET, LBRACE, RBRACE, 
