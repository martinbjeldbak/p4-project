...
content. The game \classref{Widget}s provide visual and interactive interfaces to
GAL.
...
several sub-\classref{Widget}s which each can contain further sub-\classref{Widget}s. 
...
\lstinputlisting[caption={Pseudocode for the \texttt{draw()} method.},
...
keyword so it can't be overwritten, and \texttt{handleDraw()} is protected, so
...
\lstinputlisting[caption={Pseudocode for the \texttt{mouseClicked()}
...
difference is that in contrast to drawing, input is handled bottom up. The
...
which contain the position at which the mouse is currently positioned at. For
...
the ones which are related to those \classref{Square}s. To help ease this process, usable
...
\classref{GridBoardWidget} which is a specialisations of \classref{BoardWidget}.
While it is not necessary at this point, it is an attempt to generalise
\classref{GridBoard} so that a future addition with new \classref{Board} types
will be easier to implement. This is further described in \secref{sec:boardtypes}.
...
then access the game object through its accessor's methods directly, without
caching it.
