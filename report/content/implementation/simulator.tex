...
we explain how the different \classref{Widget}s communicate with eachother in
...
which manage the position and size of its sub-\classref{Widget}s. By using
...
all its sub-\classref{Widget} and their sub-\classref{Widget}s. To do this,
\classref{Widget} has two methods, \texttt{draw()} and \texttt{handleDraw()}.
\texttt{handleDraw()} needs to be overridden in sub-\classref{Widget}s which
