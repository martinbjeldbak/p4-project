...
\begin{dlist}
  \item \constant{addAction}[\variable{piece} : \type{Piece}, \variable{squares} : \type{List}] : \type{List}\\
    A function that returns a list of squares \variable{squares} to where it's possible to add a piece (\variable{piece}. The functions
...
\begin{dlist}
  ...
  Creates a instance of the \type{Game} type with a Game title of \variable{title}, \constant{board} set to \constant{initialBoard} and \constant{currentPlayer} set to \constant{turnOrder}[0]. The \type{Game} type contains all information to describe a board game at a specific point at time.
  ...
  \type{List} of all aplied \type{Action}s.
...
\begin{dlist}
  ...
  \item \content{remove}[] : \type{Piece}\\
  A \type{Piece} where \constant{position} is invalid and \constant{onBoard} is false.
  
  \item \content{onBoard} : \type{Boolean}\\
  Is true if \type{Piece} is on the \type{GridBoard}.
  
  \item \content{actions}[ \variable{game} : \type{Game} ] : \type{List}\\
  A \type{List} of possible \type{Action}s the \type{Piece} can make on its \content{owner}'s turn.
\end{dlist}
