...
\begin{dlist}
  \item \type{String}[\variable{string} : \type{String}]\\
    ...
    ``\textbackslash{}u0108'', which is the capital C with circumflex, Äˆ). The
...
\begin{dlist}
  ...
  \item \constant{sort}[\variable{comparator} : \type{Function}] : \type{List} \\
    ...
    \todo{ascending = .\constant{sort}\texttt{[\#[}\variable{a},
    \variable{b}\texttt{] =>} \variable{a} \texttt{-} \variable{b}\texttt{]}?}
...
\begin{dlist}
  \item \type{Direction}[\variable{direction} : \type{Direction}]\\
    ...
    compared to a vector. There are eight different directions: \identifier{n}
    (north), \identifier{s} (south), \identifier{w} (west), \identifier{e}
    (east), \identifier{nw}, \identifier{ne}, \identifier{sw}, \identifier{se}.
...
\end{dlist}
\todo{er \literal{ABCD1234} en coordinate?}
...
\begin{dlist}
  \item \constdef{call}{[\farg{parameters}{List}]}{\opstar}\\
    Calls the function with the specified parameter list. \todo{Is this done?}
\end{dlist}
...
\begin{dlist}
  ...
  List of all \type{Player} types which are a part of this game.
  ...
  A \type{Game} where \constant{board} has been reset to its state before
  \type{Action} was applied and with \constant{history} updated accordingly.
