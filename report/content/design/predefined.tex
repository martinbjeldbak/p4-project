...
\begin{dlist}
  ...
  errors, which doesn't show until at run-time. The List type is similar to the \classref{ArrayList} of Java and it's resizeable, which means that types can be added to the List. The type comes with a number of built-in constants and functions. 
  \item \constant{size} : \type{Integer}\\
  ...
  \item \constant{sort}[\variable{comparator} : \type{Function}] : \type{List} \\
    The sort function sorts a list using a function that must take two parameters as input and return an integer value. For example \textt{[1, 6, 2, 5, 4, 3].sort[\#[\$a, \$b] $=>$ if \$a > \$b then 1 else if \$a == \$b then 0 else -1]}. Will sort the list in ascending order. That is \texttt{[1, 2, 3, 4, 5, 6]}.
