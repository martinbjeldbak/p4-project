...
An example of a pattern is \text{/n n e empty/}. This pattern can be checked on
the board seen in \figref{patternboard} on the field \textbf{A1}. The pattern
...
in \figref{patternboard}. For each operator, two examples of a pattern check on
...
Notice that the *-operator causes many branches to be made. The previous pattern
...
The keyword \texttt{foe} does the opposite of \texttt{friend}. It makes a branch
...
\figref{fig:patternboard}.  If we for any piece specify that it can move to a
