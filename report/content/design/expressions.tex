...
\secref{sec:setexpressions}. We also provied big-step semantics for set
...
In the second line the \variable{adder} is called as a function, which means
that a new scope, $B$, is created, in which the variable \variable{b} is
...
\secref{subsec:llparsersandlrparsers}), but preserve left-associativity. The
